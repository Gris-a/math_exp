...
#define DIF_POW if(VAL_T(node->right->type))\
                {\
                    return __MUL(__POW(CPY(node->left), __VAL(node->right->data.num - 1)),\
                                 __MUL(__VAL(node->right->data.num), DIF(node->left)));\
                }\
                else\
                {\
                    return __MUL(CPY(node), __ADD(__DIV(__MUL(CPY(node->right), DIF(node->left)), CPY(node->left)),\
...
#define DIF_LN  return __MUL(__POW(CPY(node->right), __VAL(-1)),\
                               DIF(node->right));
...
#define SMPL_POW if((VAL_T(node->left->type)  && DBL_EQ(node->left->data.num , 1)) ||\
                    (VAL_T(node->right->type) && DBL_EQ(node->right->data.num, 0)))\
                 {\
                     copy = __VAL(1);\
                 }\
